...
export const replicateImageAdapter = {
  ...
  async generateImage(input: ImageGenerationInput): Promise<ImageGenerationOutput> {
    ...
    // input.model n'est honoré que s'il ressemble à un id Replicate — jamais
    // un id fal.ai (fal-ai/…, bytedance/…) reçu en fallback de chaîne.
    const model = (input.model && input.model.includes('/') && !/^(fal-ai|bytedance|ideogram|xai|google|openai)\//.test(input.model))
      ? input.model
